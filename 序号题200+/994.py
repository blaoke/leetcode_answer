#首先遍历一遍,如果值为2就将其入队,如果值为1就将记录其中数值为1的个数cnt加1,
#然后开始出队,出队并访问上下左右4个方向上的值,如果有值为1,就cnt-1,并使这个值为2,将坐标入队,遍历一遍队列上的元素就令感染次数的值res+1,直至队列为空就退出.
#最后如果cnt>0,表示有未感染的橘子,返回-1,否则返回res
class Solution:
